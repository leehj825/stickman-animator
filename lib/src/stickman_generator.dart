...
class StickmanGenerator {
  ...
  // --- HELPER: Spherical Orbit (Rotation based on parent) ---
  // Returns a position relative to 'center' at 'dist' distance, rotated by yaw (Y) and pitch (X).
  // 0,0 points toward +Z (Front).
  static v.Vector3 _getOrbit(v.Vector3 center, double dist, double yaw, double pitch) {
    // Start with Forward vector (+Z)
    v.Vector3 offset = v.Vector3(0, 0, dist);

    // Apply Pitch (X-axis rotation)
    if (pitch != 0) {
      offset = v.Quaternion.axisAngle(v.Vector3(1, 0, 0), pitch).rotate(offset);
    }
    // Apply Yaw (Y-axis rotation)
    if (yaw != 0) {
      offset = v.Quaternion.axisAngle(v.Vector3(0, 1, 0), yaw).rotate(offset);
    }

    return center + offset;
  }

  // --- HELPER: Position Head based on Neck ---
  static void _positionHead(StickmanSkeleton pose, double dist, double yaw, double pitch) {
    if (pose.head != null) {
       pose.head!.setFrom(_getOrbit(pose.neck, dist, yaw, pitch));
    }
  }

  // --- HELPER: Solve Joint Position (IK with Swivel) ---
  static void _setLimbIK(
      StickmanSkeleton pose,
      String side, // 'l' or 'r'
      String limb, // 'Arm' or 'Leg'
      v.Vector3 targetPos,
      v.Vector3 bendHint, // Preferred direction for bending (Pole Vector)
      double len1, // Upper Limb Length
      double len2, // Lower Limb Length
      {double swivel = 0.0} // NEW: Rotation around the limb axis
  ) {
    // 1. Identify Nodes
    ...
    // 3. Calculate Plane Orientation
    v.Vector3 armAxis = dir.normalized();

    // Base Normal from Hint
    v.Vector3 bendNormal = armAxis.cross(bendHint).normalized();
    if (bendNormal.length < 0.001) bendNormal = v.Vector3(1, 0, 0);

    // NEW: Apply Swivel Rotation to the Plane Normal
    // Rotates the plane around the main axis (Hip->Foot or Neck->Hand)
    if (swivel != 0) {
      v.Quaternion swivelQ = v.Quaternion.axisAngle(armAxis, swivel);
      bendNormal = swivelQ.rotate(bendNormal);
    }

    // 4. Calculate Joint Position
  ...
  static StickmanClip generateRun(StickmanSkeleton? style) {
    ...
    for (int i = 0; i < totalFrames; i++) {
      ...
      // Knee bend toward BACK (-Z)
      _setLimbIK(pose, 'l', 'Leg', lTarget, v.Vector3(0, 0, -1), lThigh, lShin, swivel: 0);

      // Right Leg
      double rPhase = angle + pi;
      v.Vector3 rTarget = _calculateRunFootPos(rPhase, isLeft: false, baseHeight: 25.0);
      _setLimbIK(pose, 'r', 'Leg', rTarget, v.Vector3(0, 0, -1), rThigh, rShin, swivel: 0);

      // --- Arm Cycle ---
      // Left Arm
      v.Vector3 lArmT = _calculateRunHandPos(rPhase, isLeft: true, spineTop: pose.neck);
      // Elbow bend toward FRONT (+Z)
      _setLimbIK(pose, 'l', 'Arm', lArmT, v.Vector3(0, 0, 1), lUpperArm, lForeArm, swivel: 0);

      // Right Arm
      v.Vector3 rArmT = _calculateRunHandPos(lPhase, isLeft: false, spineTop: pose.neck);
      _setLimbIK(pose, 'r', 'Arm', rArmT, v.Vector3(0, 0, 1), rUpperArm, rForeArm, swivel: 0);
  ...
  static StickmanClip generateJump(StickmanSkeleton? style) {
    ...
    for (int i = 0; i < totalFrames; i++) {
      ...
      double hipY = 0;
      if (t < 0.2) { hipY = _lerp(0, 15, t/0.2); }
      else if (t < 0.5) { hipY = _lerp(15, -20, (t-0.2)/0.3); }
      else if (t < 0.8) { hipY = _lerp(-20, 0, (t-0.5)/0.3); }
      else { hipY = _lerp(0, 0, (t-0.8)/0.2); }

      pose.hip.y = hipY;
      pose.neck.setValues(0, hipY - spineLen, 0);

      double footY = 25;
      if (t > 0.3 && t < 0.8) footY = hipY + 25 - 5;
      ...
      else if (t < 0.5) { armZ = 15; armY = -15; } // Front/Up (+Z)
      else { armZ = 0; armY = 0; }
  ...
  static StickmanClip generateKick(StickmanSkeleton? style) {
    ...
    for (int i = 0; i < totalFrames; i++) {
      ...
      v.Vector3 lFoot = v.Vector3(-5, 25, -5); // Left foot Back (-Z)
      v.Vector3 rFoot = v.Vector3(5, 25, 5); // Right foot Front (+Z)

      if (t < 0.3) { // Chamber
         double pt = t/0.3;
         hip.z = _lerp(0, -5, pt); // Lean Back (-Z)
